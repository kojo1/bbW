...
static uint8_t keyExchange[] = {
    0xe7, 0x62, 0xa7, 0x5a, 0x88, 0xe1, 0xe9, 0x21,
    0xc9, 0x4a, 0x80, 0x0a, 0x0e, 0x65, 0x51, 0xd7,
    0xfc, 0xc8, 0x55, 0xdf, 0xf7, 0xb2, 0xe8, 0x9d,
    0xa6, 0xb5, 0x60, 0x6b, 0xba, 0xc0, 0x0f, 0x2f};

static int getExtLenKeyShare(void)
{
    return 10 + sizeof(keyExchange);
}

YT_LOCAL uint8_t *yt_setKeyShare(uint8_t *buff)
{
    buff = yt_setExtentionHeader(buff, YT_KEY_SHARE, 38);
    buff = yt_set2bytes(buff, 36);
    buff = yt_set2bytes(buff, YT_X25519);
    buff = yt_set2bytes(buff, 32);
    XMEMCPY(buff, keyExchange, sizeof(keyExchange));

    return buff + sizeof(keyExchange);
}
